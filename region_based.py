...
def get_views(panorama_height, panorama_width, window_size=64, stride=4):
    panorama_height /= 8
    panorama_width /= 8
...
class MultiDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def save_temp_result(self, latent, i):
        #imgs = self.decode_latents(latent)  # [1, 3, 512, 512]
        img = T.ToPILImage()(latent[0].cpu())
        ...
        with torch.autocast('cuda'):
            for i, t in enumerate(self.scheduler.timesteps):
                ...
                for h_start, h_end, w_start, w_end in views:
                    ...
                    if i == 0:
                        #self.save_temp_result(latents_view_denoised, k)
                        k += 1
...
def main(opt):
    ...
    fg_masks = torch.cat(
        [preprocess_mask(mask_path, opt.H // 8, opt.W // 8, device) for mask_path in opt.mask_paths])
    bg_mask = 1 - torch.sum(fg_masks, dim=0, keepdim=True)
    bg_mask[bg_mask < 0] = 0
    masks = torch.cat([bg_mask, fg_masks])

    prompts = [opt.bg_prompt] + opt.fg_prompts
    neg_prompts = [opt.bg_negative] + [opt.fg_negative for i in range(len(fg_masks))]

    print(f'seed: {opt.seed}')
    img = sd.generate(masks, prompts, neg_prompts, opt.H, opt.W, opt.steps, bootstrapping=opt.bootstrapping)

    # save image
    img.save(join(opt.outdir, f'{opt.seed}.jpg'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fg_negative', type=str)  # 'artifacts, blurry, smooth texture, bad quality, distortions, unrealistic, distorted image'
